...
export async function createSlot(
  ...
  if (!serviceId || !date || !startTime) {
    return { error: "Service, date, and start time are required." };
  }
  ...
  try {
    ...
    // Collides with a slot that already exists at this exact
    // service/time/format — most often one already generated by a
    // recurring rule. Report it instead of letting the unhandled
    // Prisma error surface as a generic server-action crash.
    if (err instanceof Prisma.PrismaClientKnownRequestError && err.code === "P2002") {
      return { error: "A slot already exists for this service, time, and format." };
    }
    throw err;
  }
